...
const evenNumbers = randomNumbers.filter((number) => number & 1);
...
const numbersBelow501 = crazyNumbers.reduce((acc, number) => (acc += number < 501), 0);
...
const gameNamesList = cart.reduce((acc, { name }) => (acc += `- ${name}\n`), "");
console.log(gameNamesList);
